...
public class CurrencyPage extends TestBase {
	...
	public CurrencyPage approveCurrency(String ISOCode) throws InterruptedException {
		currencyIsoCode.click();
		Select sl = new Select(currencyIsoCode);
		/*String EleNotFound=errormessages.getText();
		String EleFailMsg="No items available for Approve operation given the search criteria";
		if(sl.getOptions().isEmpty() && EleNotFound.equals(EleFailMsg))
		{
			System.out.println("Required Element Not Found");
			System.out.println("Approved action already performed for " +ISOCode);
		}*/
	...
	public CurrencyPage listCurrency1() throws InterruptedException {
		...
		if(a.equals(b)) {
			System.out.println("No items available for List operation given the search criteria messages is displayed on screen");
		}else {
			System.out.println("Validation failed when listing deleted currency");
		}
		logOutBtn.click();
		logoutBtn.click();
		driver.quit();
	...
	public CurrencyPage modifyCurrencywithInvalidddata(String ISOCode, String CurrencyUnit,
		...
		if (a.equals(b)) {
			...
			logoutBtn.click();
			driver.quit();
		} else if(a.contains(c) && a.contains(d) && a.contains(b)) {
			System.out.println("Validation Message is proper for ConvRate,ConvTolerance and Currency Unit " + ISOCode + " currency");
			logOutBtn.click();
			logoutBtn.click();
			driver.quit();
		}
