...
public class ChequeStopPage extends TestBase {
	...
	public ChequeStopPage addChequeStopentrywithBlankdata(String Issuer, String BranchCode,
		...
		if (g.equals(h)) {
			...
			System.out.println("Validation message is displayed for Currency field");
			logOutBtn.click();
			logoutBtn.click();
			driver.quit();
		} else if (g.equals(j)) {
			System.out.println("Validation message is displayed for Minimum and Maximum Cheque number field for " + Currency + "");
			logOutBtn.click();
			logoutBtn.click();
			driver.quit();
		} else if (g.equals(k)) {
